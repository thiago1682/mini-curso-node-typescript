...
locationsRouter.get("/", async (request, response) => {
  ...
  if (items) {
    parsedItems = <any>String(items)
      .split(",")
      .map((item) => Number(item.trim()));
  }

  // inicia a consulta
  let location = knex("locations");

  // verifica se city está presente no request.query e adiciona à consulta
  if (city) {
    location = location.where("city", String(city));
  }

  // verifica se uf está presente no request.query e adiciona à consulta
  if (uf) {
    location = location.where("uf", String(uf));
  }

  // verifica se items está presente no request.query e adiciona à consulta
